...
pub fn generate_sourcemap(lulu_source: &str, lua_output: &str) -> Vec<SourceMapEntry> {
  ...
  // Optional: fill in gaps between mapped chars to cover inserted lines
  let mut filled = Vec::new();
  let mut last: Option<SourceMapEntry> = None;
  for entry in sourcemap {
    if let Some(prev) = last {
      // interpolate Lua columns/lines between prev and entry
...
pub fn lookup_lua_to_lulu(
  ...
  // find the entry that matches or is closest
  sourcemap
    .iter()
